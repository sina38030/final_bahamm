...
export default function StickyHeader({close}:{close:()=>void}){
  ...
  useEffect(()=>{
    ...
    const onScr=()=>setShow(modal.scrollTop>80);
    modal.addEventListener('scroll',onScr);
  ...
  return(
    <header className={`${styles.stickyBar} ${show?styles.barShow:''}`}>
      ...
      <span className={styles.msg} onClick={()=>open('headline')}>{msgs[idx]}</span>
      <button className={styles.smallBtn} onClick={()=>open('share')}>
