...
export async function GET(
  ...
  try {
    ...
    const getItemGroupPrice = (item: any, friends: number): number => {
      ...
      const f1 = (() => { const v = toNum(item?.friend_1_price); return v > 0 ? v : Math.round(solo / 2); })();
      const f2 = (() => { const v = toNum(item?.friend_2_price); return v > 0 ? v : Math.round(solo / 3); })();
      const f3 = (() => { const v = toNum(item?.friend_3_price); return v > 0 ? v : 0; })();
    ...
    if (isSecondaryGroup) {
      ...
      alone = totalBasketValue;
      friend1Total = totalBasketValue - (1 * (totalBasketValue / 4));
      friend2Total = totalBasketValue - (2 * (totalBasketValue / 4));
      const friend3Total = totalBasketValue - (3 * (totalBasketValue / 4));
      ...
      else if (paidNonLeaders >= 3) leaderPrice = 0;
    ...
    const originalTotal = alone;
    const currentTotal = leaderPrice;
    const expectedTotal = expectedLeaderPrice;
