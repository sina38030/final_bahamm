...
function CheckoutPageContent() {
  ...
  // Use admin-configured next-day delivery slots for invited users and solo purchases
  const useAdminTomorrowSlots = isInvitedUser || actualMode === 'solo';
  ...
  // Fetch next day delivery slots from admin when required (invited or solo)
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Set simple label for checkout page
        setTomorrowLabelPrefix('فردا');

        // Fetch the next two days of slots and select the exact Tehran tomorrow
        const slotsRes = await fetch(`${API_BASE_URL}/admin/delivery-slots/next?days=2`, { cache: 'no-store' });
        const slotsJson = await slotsRes.json().catch(() => ({}));
        const daysArr: any[] = Array.isArray(slotsJson?.days) ? slotsJson.days : [];

        // Prefer explicit date match; fallback to second item
        let tomorrow: any = daysArr.find((d: any) => String(d?.date) === tIso) || daysArr[1] || null;
        ...
        if (tomorrow?.day_off) {
          ...
          setSelectedSlot('فردا تعطیل است');
          return;
        }

        const activeSlots: Array<{ start_time: string; end_time: string }> = (tomorrow?.slots || [])
        ...
        if (!hasSetDefaultSlot) {
          if (labels.length > 0) {
            setSelectedSlot(`${tomorrowLabelPrefix} ${labels[0]}`);
            setChosenDay(0);
            setChosenSlot(0);
          } else {
            setSelectedSlot('فردا بازه‌ای در دسترس نیست');
          }
    ...
    <div style={{ minHeight: '100vh', background: '#fafafa', direction: 'rtl' }}>
      ...
      <section style={{
        ...
        <div style={{
          ...
          {isLeaderGroup && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px' }}>
              <span>زمان ارسال رو بعد از تشکیل گروه تعیین خواهید کرد</span>
              <button
                onClick={() => setShowLeaderShipInfoPopup(true)}
                aria-label="توضیحات زمان ارسال"
                style={{
                  width: '18px',
                  height: '18px',
                  borderRadius: '50%',
          ...
          {/* Delivery time: for invited + consolidation ON, hide picker and show message; otherwise show as before (except for group leaders) */}
          {(isInvitedUser && greenToggle) ? (
            ...
            !(actualMode === 'group' && !isInvitedUser) && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
